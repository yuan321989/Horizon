...
def _prepare_variables_for_body(
    ...
    Long summaries are split into multiple messages in notify()
    rather than truncated here, so all content reaches the user.
    """
...
class WebhookNotifier:
    ...
    async def notify(self, variables: dict) -> None:
        ...
        # --- Long text chunking: split body at line breaks if text exceeds 3900 chars ---
        chunks: list[str] = []
        try:
            parsed = json.loads(body_content) if body_content else {}
            if isinstance(parsed, dict) and "text" in parsed and len(parsed["text"]) > 3900:
                self.console.print(
                    f"[dim]📎 Summary is {len(parsed['text'])} chars, "
                    f"splitting into multiple messages...[/dim]"
                )
                chunk_size = 3900
                text = parsed["text"]
                lines = text.split("\n")
                current_chunk = ""
                for line in lines:
                    # Start a new chunk when adding this line would exceed the limit
                ...
                if current_chunk:
                    payload = dict(parsed)
                    payload["text"] = current_chunk
                    payload["disable_notification"] = len(chunks) > 0  # silent for follow-ups
                    chunks.append(json.dumps(payload, ensure_ascii=False))
            else:
                chunks = [body_content] if body_content else []
                ...
                for i, chunk_body in enumerate(chunks):
                    if chunk_body is None:
                        ...
                        response = await client.post(
                            request_url,
                            content=chunk_body.encode("utf-8"),
                            headers=headers,
                        )
    ...
    async def send_daily_summary(
        ...
        # Also upload the full summary file as a document (Telegram sendDocument)
        if summary_path and self.url:
            try:
                import pathlib
                path = pathlib.Path(summary_path)
                if path.exists():
                    # Build sendDocument URL from existing webhook URL
                    base = self.url.split("/sendMessage")[0]
                    doc_url = f"{base}/sendDocument"

                    file_name = f"horizon-{date}-{lang}.md"
                    caption = (
                        f"📰 Horizon {date} 完整日报 ({lang.upper()})"
                        if lang == "zh"
                        else f"📰 Horizon {date} Full Report ({lang.upper()})"
                    )

                    async with httpx.AsyncClient(timeout=30.0) as client:
                        with open(path, "rb") as f:
                            # Extract chat_id from webhook URL: .../sendMessage?chat_id=7667670199
                            chat_id = (
                                self.url.split("chat_id=")[-1].split("&")[0]
                                if "chat_id=" in self.url
                                else "7667670199"
                            )
                            response = await client.post(
                                doc_url,
                                ...
                                files={
                                    "document": (file_name, f, "text/markdown"),
                                },
                            )
                        result = response.json()
                        if result.get("ok"):
                            self.console.print(
                                f"[green]📄 Full report uploaded as document[/green]"
                            )
                        else:
                            logger.warning(
                                "Webhook document upload failed: %s",
                                result.get("description", ""),
                            )
            except Exception as e:
                logger.debug("Webhook document upload skipped: %s", e)
